            ...
            <div className="flex flex-col w-full mt-4 md:mt-4">
              {Array.isArray(sortedTeams) && sortedTeams.length > 0 ? (
                sortedTeams.map((team, index) => (
                  <div key={team.nome} className="flex flex-row justify-start items-center ml-4 md:mx-4 mb-4 border-b border-white pb-4">
                    ...
                    <div className="flex flex-col w-full md:w-full md:flex-row md:pl-5 h-full justify-center md:justify-start items-center">
                      <img src={team.url_image} alt={`${team.nome}-logo`} className="flex rounded-full ring-2 ring-white min-w-10 h-10 md:w-14 md:h-14"/>
                      <h3 className="text-white font-SuperDario text-md md:text-xl md:ml-3">{team.nome.toUpperCase()}</h3>
                    </div>
                    ...
                    <h3 className="text-white h-6 md:h-10 min-w-[55.5px] mr-3 text-center md:min-w-[83px] md:text-center font-SuperDario text-2xl md:text-4xl">{team.total_pontos}</h3>
                  </div>
                ))
              ) : (
                <Loading/>
